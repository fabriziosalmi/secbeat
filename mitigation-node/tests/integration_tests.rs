...
use mitigation_node::config::{MitigationConfig, ManagementApiConfig};
    ...
    async fn test_add_waf_pattern_endpoint() {
        ...
        match response {
            Ok(resp) => {
                ...
                let result = waf.check_custom_patterns("This contains malicious_test_pattern_123").await;
    ...
    async fn test_remove_waf_pattern_endpoint() {
        ...
        match response {
            Ok(resp) => {
                ...
                let waf = waf_engine.read().await;
                let result = waf.check_custom_patterns("This contains test_remove_pattern").await;
...
"#;
        ...
        let reloaded_config = MitigationConfig::from_file(temp_file.path().to_str().unwrap()).unwrap();
    ...
    async fn test_environment_variable_integration() {
        ...
        assert_eq!(config.management.auth_token, Some("env-token-123".to_string()));
...
mod protection_integration_tests {
    ...
    async fn test_blacklist_integration() {
        ...
        // Add IP to blacklist
        ddos.add_to_blacklist(malicious_ip, Duration::from_secs(300)).await;
    ...
    async fn test_dynamic_rule_application() {
        ...
        waf.add_custom_pattern("suspicious_pattern_xyz").await.unwrap();
        ...
        // Remove the rule
        let removed = waf.remove_custom_pattern("suspicious_pattern_xyz").await.unwrap();
        assert_eq!(removed, 1);
...
mod event_integration_tests {
    use super::*;
    use mitigation_node::events::{EventConfig, EventSystem, SecurityEvent, EventSeverity};
    ...
    async fn test_event_publishing_workflow() {
        ...
        match EventSystem::new(nats_url, node_id).await {
            ...
            Err(e) => {
                println!("Event system initialization failed (expected without NATS): {}", e);
...
mod performance_integration_tests {
    ...
    async fn test_concurrent_waf_requests() {
        ...
        println!("Concurrent WAF Performance: {:.2} requests/second", requests_per_second);
    ...
    async fn test_memory_usage_under_load() {
        ...
        println!("Memory usage: {} MB -> {} MB (increase: {} MB)", 
                initial_memory, final_memory, memory_increase);
        
        // Memory increase should be reasonable (less than 50MB for this test)
        assert!(memory_increase < 50.0,
               "Memory increase should be less than 50MB, got {} MB", memory_increase);
    }
...
mod e2e_workflow_tests {
    ...
    async fn test_complete_protection_pipeline() {
        ...
        let test_cases = vec![
            ...
            ("sql_injection", "192.168.1.2", "/api/login", "username=admin'; DROP TABLE users; --", false),
            ("xss_attack", "192.168.1.3", "/api/comment", "comment=<script>alert('xss')</script>", false),
            ("path_traversal", "192.168.1.4", "/api/file", "path=../../../etc/passwd", false),
        ];
        ...
        for (test_name, ip_str, path, payload, should_allow) in test_cases {
            ...
            if waf_guard.check_sql_injection(payload).await == mitigation_node::waf::WafResult::Blocked {
                blocked = true;
            ...
            } else if waf_guard.check_path_traversal(payload).await == mitigation_node::waf::WafResult::Blocked {
                blocked = true;
            ...
            assert_eq!(actually_allowed, should_allow,
