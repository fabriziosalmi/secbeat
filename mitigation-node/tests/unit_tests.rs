...
mod ddos_tests {
    ...
    async fn test_ddos_protection_initialization() {
        ...
        assert!(
            ddos.is_ok(),
            "DDoS protection should initialize successfully"
        );
    }
...
mod performance_tests {
    ...
    async fn test_waf_performance() {
        ...
        println!(
            "WAF Performance: {:.2} requests/second",
            requests_per_second
        );
    ...
    async fn test_ddos_rate_limiting_performance() {
        ...
        println!(
            "DDoS Rate Limiting Performance: {:.2} checks/second",
            checks_per_second
        );

        // Expect at least 5000 checks per second
        assert!(
            checks_per_second > 5000.0,
...
mod security_tests {
    ...
    async fn test_attack_simulation() {
        ...
        println!(
            "Attack Detection Rate: {:.1}% ({}/{} attacks blocked)",
        ...
        assert!(
            block_rate >= 80.0,
            "WAF should block at least 80% of attacks, got {:.1}%",
            block_rate
        );
    }
