...
mod waf_performance_tests {
    ...
    async fn test_waf_baseline_performance() {
        ...
        assert!(avg_latency < 1.0, "Average latency should be < 1ms, got {:.2}ms", avg_latency);
        assert!(p95_latency < 5.0, "P95 latency should be < 5ms, got {:.2}ms", p95_latency);
        assert!(rps > 10000.0, "Should handle > 10k requests/sec, got {:.2}", rps);
    }
    
    #[tokio::test]
    async fn test_waf_concurrent_performance() {
        let config = MitigationConfig::default();
        let waf = Arc::new(RwLock::new(WafEngine::new(config.waf.clone()).await.unwrap()));
        ...
        assert!(results.avg_latency_ms < 10.0, 
               "Concurrent avg latency should be < 10ms, got {:.2}ms", results.avg_latency_ms);
        assert!(results.requests_per_second > 1000.0,
               "Should handle > 1k concurrent requests/sec, got {:.2}", results.requests_per_second);
    ...
    async fn test_waf_memory_efficiency() {
        ...
        println!("  After {} patterns: {:.2} MB", pattern_count, after_patterns_memory);
        println!("  After {} checks: {:.2} MB", iterations, final_memory);
        println!("  Memory per pattern: {:.2} KB", 
                (after_patterns_memory - initial_memory) * 1024.0 / pattern_count as f64);
        
        let rps = iterations as f64 / duration.as_secs_f64();
        println!("  Performance with {} patterns: {:.2} req/sec", pattern_count, rps);
        
        // Memory efficiency assertions
        let memory_increase = final_memory - initial_memory;
        assert!(memory_increase < 50.0, 
               "Memory increase should be < 50MB, got {:.2}MB", memory_increase);
        assert!(rps > 1000.0,
               "Should maintain > 1k req/sec with many patterns, got {:.2}", rps);
    }
...
mod ddos_performance_tests {
    ...
    async fn test_ddos_rate_limiting_performance() {
        ...
        assert!(checks_per_second > 50000.0,
               "Should handle > 50k rate limit checks/sec, got {:.2}", checks_per_second);
    }
    ...
    async fn test_ddos_concurrent_ip_tracking() {
        ...
        for ip_suffix in 0..concurrent_ips {
            ...
            tasks.spawn(async move {
                let client_ip: IpAddr = format!("192.168.{}.{}", 
                    ip_suffix / 256, ip_suffix % 256).parse().unwrap();
        ...
        assert!(rps > 5000.0,
               "Should handle > 5k concurrent IP requests/sec, got {:.2}", rps);
    }
    ...
    async fn test_ddos_blacklist_performance() {
        ...
        assert!(lookups_per_second > 100000.0,
               "Should handle > 100k blacklist lookups/sec, got {:.2}", lookups_per_second);
    }
}
...
mod stress_tests {
    ...
    async fn test_system_under_extreme_load() {
        ...
        for user_id in 0..extreme_config.concurrent_users {
            ...
            tasks.spawn(async move {
                let client_ip: IpAddr = format!("192.168.{}.{}", 
                    user_id / 256, user_id % 256).parse().unwrap();
        ...
        assert!(avg_latency < 50.0, 
               "Avg latency under extreme load should be < 50ms, got {:.2}ms", avg_latency);
        assert!(p95_latency < 100.0,
               "P95 latency under extreme load should be < 100ms, got {:.2}ms", p95_latency);
        assert!(rps > 500.0,
               "Should maintain > 500 req/sec under extreme load, got {:.2}", rps);
    }
    ...
    async fn test_memory_leak_detection() {
        ...
        assert!(total_memory_increase < 50.0,
               "Total memory increase should be < 50MB, got {:.2}MB", total_memory_increase);
    }
...
mod scalability_tests {
    ...
    async fn test_linear_scalability() {
        ...
        for &concurrent_users in &load_levels {
            ...
            println!("Load level {} users: {:.2} req/sec, {:.2}ms avg latency",
        ...
        for i in 1..results.len() {
            ...
            let throughput_ratio = curr_result.requests_per_second / prev_result.requests_per_second;
            
            println!("Scaling {}x users: throughput scaled by {:.2}x", 
                    user_ratio, throughput_ratio);
            
            // Throughput should scale reasonably (at least 50% of linear scaling)
            assert!(throughput_ratio > user_ratio * 0.5,
                   "Throughput scaling should be at least 50% linear, got {:.2}x for {:.2}x users",
                   throughput_ratio, user_ratio);
        }
    }
